"""
Pokemon TCG v4 — Web GUI Backend
Flask server bridging the v4 game engine and champion RL agents to a browser UI.
...
# ── Paths ───────────────────────────────────────────────────────────────────
OUT_DIR        = os.path.join(_V4_DIR, "outputs")
LEADERBOARD    = os.path.join(OUT_DIR, "leaderboard.json")
CHAMP_P0_PATH  = os.path.join(OUT_DIR, "champion_p0.npy")   # best P0 agent (Starmie)
CHAMP_P1_PATH  = os.path.join(OUT_DIR, "champion_p1.npy")   # best P1 agent (Lucario)
# Legacy fallbacks for old 2-agent saves
_LEGACY_CHAMPS = {
    0: os.path.join(OUT_DIR, "ppo_lucario_champion.npy"),
    1: os.path.join(OUT_DIR, "dqn_starmie_champion.npy"),
}

IMAGES_DIRS = {
    "CardImagesStarmie":  os.path.join(_V4_DIR, "CardImagesStarmie"),
    "CardImagesLucario":  os.path.join(_V4_DIR, "CardImagesLucario"),
}
...
ENERGY_COLORS = {
    EnergyType.COLORLESS: "#aaa",
    EnergyType.FIGHTING:  "#c87830",
    EnergyType.WATER:     "#40b8e8",
    EnergyType.DARKNESS:  "#605088",
    EnergyType.PSYCHIC:   "#c070e8",
    EnergyType.LIGHTNING: "#f0d020",
}

STAGE_NAMES = {Stage.BASIC: "Basic", Stage.STAGE1: "Stage 1", Stage.STAGE2: "Stage 2"}

# Explicit overrides where card name → image base differ
_IMAGE_OVERRIDES = {
    # Energy cards
    "Water Energy":           "Basic_Water_Energy",
    "Darkness Energy":        "Basic_Dark_Energy",
    "Fighting Energy":        "Basic_Fighting_Energy",
    # Pokémon (strip " ex" suffix)
    "Mega Starmie ex":        "Mega_Starmie",
    "Mega Lucario ex":        "Mega_Lucario",
    "Bloodmoon Ursaluna ex":  "Bloodmoon_Ursaluna",
    "Fezandipiti ex":         "Fezandipiti",
    "Latias ex":              "Latias",
    "Meowth ex":              "Meowth",
    # Trainers with special chars
    "Poké Pad":               "Poke_Pad",
    "Pokégear 3.0":           "Pokegear",
    "Buddy-Buddy Poffin":     "Buddy_Buddy_Poffin",
    "Lillie's Determination": "Lillies_Determination",
    "Boss's Orders":          "Boss_Orders",
    "Wally's Compassion":     "Wally_Compassion",
    "Team Rocket's Petrel":   "Team_Rockets_Petrel",
    "Black Belt's Training":  "Black_Belt_Training",
}


def _img(name: str) -> str:
    """Return URL for a card image, searching both v4 image directories."""
    base = _IMAGE_OVERRIDES.get(name) or name.replace(" ", "_")
    for dir_key, dir_path in IMAGES_DIRS.items():
        for ext in (".png", ".jpg", ".jpeg"):
            if os.path.exists(os.path.join(dir_path, base + ext)):
                return f"/images/{dir_key}/{base}{ext}"
    return ""
...
def _serialize_pokemon(p: PokemonCard | None, index: int = -1) -> dict | None:
    ...
    return {
        ...
        "ability_effect": p.ability_effect,
        "ability_type":  p.ability_type,
        "ability_used":  p.ability_used,
        "shadow_bound":  p.shadow_bound,
        "cant_attack":   p.cant_attack_turns > 0,
        "is_ex":         p.is_ex,
        "is_mega_ex":    p.is_mega_ex,
    }
...
def _serialize_card(card, hand_idx: int) -> dict:
    ...
    if isinstance(card, EnergyCard):
        return {
            ...
            "img":         _img(card.name),
            "index":       hand_idx,
            "special_effect": card.special_effect or "",
        }
    if isinstance(card, TrainerCard):
        ...
        ct = ct_map.get(card.card_type, "item")
        return {
            "card_type": ct,
            "name":      card.name,
            "effect":    card.effect,
...
def build_state_json() -> dict:
    ...
    def my_target_desc(slot: int) -> dict:
        if slot == 0:
    ...
    # ── legal_hand: hand-indexed actions ────────────────────────────────
    legal_hand: dict[int, list] = {}

    for atype in (ActionType.PLAY_POKEMON, ActionType.USE_ITEM,
                  ActionType.USE_SUPPORTER, ActionType.EVOLVE, ActionType.USE_STADIUM):
        for aidx, params in by_type[atype]:
            ...
            if atype == ActionType.EVOLVE:
                slot = params["slot"]
                ui_slot = 0 if slot == 5 else slot + 1
                entry["target"] = my_target_desc(ui_slot)
            legal_hand.setdefault(hi, []).append(entry)

    # Energy attach → legal_hand (all energy cards)
    ...
    legal_abilities: dict[int, int] = {}   # slot → action_idx
    legal_retreat:   dict[int, int] = {}   # bench_slot → action_idx
    ...
    for aidx, params in by_type[ActionType.USE_ABILITY]:
        legal_abilities[params["slot"]] = aidx
    for aidx, params in by_type[ActionType.RETREAT]:
        legal_retreat[params["bench_slot"]] = aidx
    ...
    return {
        ...
        "active_stadium": gs.active_stadium.name if gs.active_stadium else None,
        "log":            g["log"][-30:],
        ...
        "my": {
            ...
            "items_blocked": me.items_blocked,
        },
        "opp": {
        ...
        "legal_abilities":    legal_abilities,
        "legal_retreat":      legal_retreat,
...
def _run_bot_turn():
    ...
    ACTION_DELAYS = {
        ActionType.ATTACK:        0.7,
        ActionType.END_TURN:      0.3,
        ActionType.EVOLVE:        0.5,
        ActionType.ATTACH_ENERGY: 0.35,
        ActionType.PLAY_POKEMON:  0.35,
        ActionType.USE_SUPPORTER: 0.45,
        ActionType.USE_ITEM:      0.35,
        ActionType.USE_ABILITY:   0.4,
        ActionType.USE_STADIUM:   0.35,
        ActionType.RETREAT:       0.4,
    ...
    while (not gs.game_over
        ...
        time.sleep(ACTION_DELAYS.get(atype, 0.35))
...
def _action_to_log(action_idx: int, gs: GameState, player_idx: int, prefix: str) -> str:
    ...
    if atype == ActionType.USE_STADIUM:
        return f"{prefix} played stadium: {hand_name(params.get('hand_idx', -1))}."
    if atype == ActionType.USE_ABILITY:
        slot = params.get("slot", 0)
        poke = me.active if slot == 0 else (me.bench[slot - 1] if 0 <= slot - 1 < len(me.bench) else None)
        pname = poke.name if poke else "?"
        aname = poke.ability_name if poke else "?"
        return f"{prefix} used {aname} ({pname})."
    if atype == ActionType.RETREAT:
        bs = params.get("bench_slot", 0)
        if 0 <= bs < len(me.bench):
            return f"{prefix} retreated; {me.bench[bs].name} sent up."
        return f"{prefix} retreated."
...
def _load_bot_champion(player_idx: int) -> PPOAgent | DQNAgent:
    """
    Load the best trained agent for the given player slot.
    Reads leaderboard.json to determine agent type (PPO or DQN),
    loads champion_p{idx}.npy, falls back to legacy champion files.
    """
    # Determine agent type from leaderboard
    agent_type = "ppo" if player_idx == 0 else "dqn"
    if os.path.exists(LEADERBOARD):
        try:
            with open(LEADERBOARD) as f:
                lb = json.load(f)
            cinfo = lb.get(f"champion_p{player_idx}", {})
            agent_type = cinfo.get("type", agent_type)
        except Exception:
            pass

    if agent_type == "ppo":
        agent = PPOAgent(player_idx=player_idx, seed=player_idx * 2)
    else:
        agent = DQNAgent(player_idx=player_idx, seed=player_idx * 2 + 1)

    # Try new-style side champion file first
    side_path = CHAMP_P0_PATH if player_idx == 0 else CHAMP_P1_PATH
    if os.path.exists(side_path):
        try:
            agent.load(side_path)
            print(f"[GUI] Loaded P{player_idx} champion ({agent_type.upper()}) "
                  f"from {os.path.basename(side_path)}")
            return agent
        except Exception as e:
            print(f"[GUI] Failed to load {side_path}: {e}")

    # Fallback: legacy champion files from old 2-agent training
    legacy = _LEGACY_CHAMPS.get(player_idx)
    if legacy and os.path.exists(legacy):
        try:
            agent.load(legacy)
            print(f"[GUI] Loaded P{player_idx} legacy champion from {os.path.basename(legacy)}")
            return agent
        except Exception as e:
            print(f"[GUI] Failed to load legacy {legacy}: {e}")

    print(f"[GUI] No champion found for P{player_idx} — using random weights")
    return agent
...
@app.route("/images/<dir_name>/<path:fname>")
def serve_image(dir_name, fname):
    img_dir = IMAGES_DIRS.get(dir_name)
    if img_dir is None:
        return "", 404
...
def new_game():
    ...
    deck_choice = str(data.get("deck", "starmie")).lower()
    if deck_choice not in ("lucario", "starmie"):
        return jsonify({"error": "Invalid 'deck' value (use 'lucario' or 'starmie')"}), 400

    # P0 = Starmie deck,  P1 = Lucario deck
    if deck_choice == "starmie":
        human_player = 0
        bot_player   = 1
    else:
        human_player = 1
    ...
    bot = _load_bot_champion(bot_player)
    ...
    with _lock:
        ...
        _game["log"]          = [f"New game! You play Mega {'Starmie' if deck_choice == 'starmie' else 'Lucario'} ex."]
        _game["pending"]      = False
        _game["mode"]         = "playing"
    ...
    with _lock:
        ...
        try:
            if atype != ActionType.ATTACK:
                ...
                if gs.last_damage_prevented:
                    _log("Bot's protection blocked all damage!")
                elif gs.last_damage_reduced:
                    _log("Bot's effect reduced your damage.")
        except Exception as e:
...
if __name__ == "__main__":
    ...
    args = p.parse_args()
    print(f"Starting Pokemon TCG v4 GUI server...")
