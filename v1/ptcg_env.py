...
def compute_legal_mask(gs: GameState) -> np.ndarray:
    ...
    Pruning decisions:
    - ATTACH_ENERGY: Only to pokemon that can use at least one attack if 1 more energy attached,
      OR to active pokemon (always useful). Prevents attaching to hopeless benched basics
      with no relevant evolutions available. We simplify: allow attaching to any pokemon.
      Prune: do not allow if no energy in hand.
    - PLAY_POKEMON: Only if bench has space (< MAX_BENCH). Only valid basics from hand.
    - EVOLVE: Only valid evolution chains, pokemon must have been in play >= 1 turn.
    - ATTACK: Only if active pokemon can pay cost.
    - USE_ITEM/SUPPORTER: Only valid cards at correct hand positions.
    - END_TURN: Always legal (fallback).
    """
...
class GameEngine:
    ...
    def _handle_ko(self, gs: GameState, defeated_player_idx: int,
        ...
        """Handle a KO: award prize counter, promote bench pokemon."""
...
class HeuristicAgent:
    ...
    Priority: Attack > Evolve > Attach Energy > Play Pokemon > Use Item > Supporter > End Turn
    """
    ...
    def act(self, env: PokemonTCGEnv) -> int:
        ...
        def has_type(t: ActionType, legal_list):
            return [a for a in legal_list
                    if ActionMapper.decode(a)[0] == t]

        # 1. Attack if possible
        attacks = has_type(ActionType.ATTACK, legal)
        if attacks:
            # Prefer higher-damage attack
            best = max(attacks, key=lambda a: me.active.attacks[ActionMapper.decode(a)[1]["atk_idx"]].damage)
            return best

        # 2. Evolve if possible
        evolves = has_type(ActionType.EVOLVE, legal)
        if evolves:
            return evolves[0]

        # 3. Attach energy to active
        attaches = has_type(ActionType.ATTACH_ENERGY, legal)
        if attaches:
            # Prefer active (slot 0)
            active_attach = [a for a in attaches if ActionMapper.decode(a)[1]["slot"] == 0]
            return (active_attach or attaches)[0]

        # 4. Play pokemon to bench
        plays = has_type(ActionType.PLAY_POKEMON, legal)
        if plays:
            return plays[0]

        # 5. Use items
        items = has_type(ActionType.USE_ITEM, legal)
        if items:
            return items[0]

        # 6. Use supporter
        supps = has_type(ActionType.USE_SUPPORTER, legal)
        if supps:
            return supps[0]
