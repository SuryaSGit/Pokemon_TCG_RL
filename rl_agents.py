...
class SelfPlayEnv:
    ...
    def step(self, action: int) -> Tuple[np.ndarray, np.ndarray, float, float, bool]:
        ...
        Reward shaping applied here:
        - Attack actions: small bonus (+0.05) to encourage attacking
        - END_TURN without having attacked: tiny penalty (-0.02) if attack was legal
        """
        cp = self.current_player
        gs = self.env.gs

        # Check if attack was legal before stepping (for shaping)
        from ptcg_env import compute_legal_mask, ActionMapper as AM, ActionType as AT
        mask_before = compute_legal_mask(gs)
        attack_was_legal = any(
            mask_before[AM.ATTACK_START + i] > 0 for i in range(2)
        )
        atype, _ = AM.decode(action)
        ...
        shape = 0.0
        if atype == AT.ATTACK:
            shape = 0.1   # encourage attacking
        elif atype == AT.END_TURN and attack_was_legal:
            shape = -0.05  # small penalty for skipping attack

        r0 = (reward + shape) if cp == 0 else -(reward + shape)
        r1 = (reward + shape) if cp == 1 else -(reward + shape)
...
if __name__ == "__main__":
    ...
    # Save trained agents
    ppo_agent.save("ppo_lycanroc.npy")
    dqn_agent.save("dqn_raichu.npy")
