...
def run_training(n_episodes: int = 500, eval_games: int = 100,
    ...
    ppo = load_agent(0)
    # Tune hyperparams for dense reward landscape
    ppo.ent_coef  = 0.03   # less entropy needed — rewards guide the policy
    ppo.lr        = 2e-4
    ppo.clip_eps  = 0.15
    ...
    while not gs.game_over:
        ...
        if cp == human_player:
            ...
            while True:
                ...
                # If it was an attack, turn switches — break inner loop
                if atype == ActionType.ATTACK or atype == ActionType.END_TURN:
                    break
                if gs.game_over:
                    break
                # Otherwise, keep picking actions
        else:
            print(f"\n  {_c('BOT THINKING...', BLD+R)}")
            time.sleep(0.4)

            # Bot acts until its turn ends
            while gs.current_player == cp and not gs.game_over:
                # Temporarily flip perspective for bot
                orig = gs.current_player
